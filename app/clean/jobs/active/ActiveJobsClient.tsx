...
export default function ActiveJobsClient({ jobs, currentTab }: Props) {
    const [viewMode, setViewMode] = useState<'list' | 'calendar'>('list');

    const calendarEvents = jobs.map(j => ({
        id: j.id,
        date: new Date(j.scheduled_at),
        title: j.spaces?.name || '청소 일정',
        ...
        <>
            ...
            {viewMode === 'calendar' ? (
                <div className="px-4 animate-in fade-in duration-300">
                    <CalendarView events={calendarEvents} onEventClick={(id) => window.location.href = `/clean/job/${id}`} />
                </div>
            ) : (
                <div className="flex-1 overflow-y-auto animate-in fade-in duration-300">
                    {jobs.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                            <span className="material-symbols-outlined text-slate-200 dark:text-slate-800 text-[60px] mb-4">event_available</span>
